...
export default function PlacementPage() {
  ...
  return (
    <>
      <BackHeader title="Kiểm tra trình độ" />
      <div className="pt-6 max-w-md mx-auto pb-12">
        ...
        {phase === 'test' && (
          <div className="space-y-5">
            <div>
              <h1 className="font-serif-display text-2xl text-ink mb-1">Bạn biết những từ nào?</h1>
              <p className="text-sm text-ink-soft">
                Tích vào từ bạn đã biết rõ nghĩa. Không cần suy nghĩ lâu — chỉ mất khoảng 1-2 phút.
              </p>
            </div>
            ...
            <Button variant="primary" onClick={submitTest} className="w-full">
              Xem kết quả ({answeredCount}/{items.length} đã biết)
            </Button>
        ...
        {phase === 'triage' && (
          <div className="space-y-5">
            <div>
              <h1 className="font-serif-display text-2xl text-ink mb-1">Vài từ đáng học ngay</h1>
              <p className="text-sm text-ink-soft">
                Trình độ ước tính của bạn: <strong className="text-ink">{level}</strong>. Xác nhận mức độ biết của
                từng từ để bắt đầu sổ tay.
              </p>
            </div>
            <TriageList
              items={candidates}
        ...
        {phase === 'done' && (
          <div className="py-16 text-center space-y-4">
            ...
            <p className="font-serif-display text-2xl text-ink">Trình độ của bạn: {level}</p>
            <p className="text-sm text-ink-soft">
              {addedCount > 0
                ? `Đã thêm ${addedCount} từ vào sổ tay, sẵn sàng để luyện tập.`
                : 'Sổ tay của bạn đã sẵn sàng.'}
            </p>
            <div className="flex flex-wrap gap-2 justify-center pt-2">
              <Link href="/today">
                <Button variant="quiet">Về trang chủ</Button>
              </Link>
              <Link href="/practice">
