...
| 9 | Kho từ vựng CEFR + phân trình độ + tự nạp từ mới — xem chi tiết dưới, ADR-014..020 | ✅ Xong (9.1-9.5), 9.6 quyết định giữ nguyên `analyzeDocument` mồ côi | `npm run typecheck && npx eslint . && npx vitest run` xanh (trừ 14 test `localStorage`/jsdom lỗi môi trường có sẵn từ trước, không liên quan); đã xác minh bằng trình duyệt thật (placement flow + auto top-up degraded path) |
...
## Phase 9 — chi tiết 6 sub-phase (ADR-014..020)

Bối cảnh: trước phase này app không có kho từ vựng nào (5 từ demo hardcode là toàn bộ nội dung tĩnh) và không có logic phân trình độ nào (`level` chỉ là dropdown tự khai, không liên hệ gì tới hiệu suất thật).

| Sub-phase | Nội dung | Trạng thái | File chính |
|---|---|---|---|
| 9.1 | `Word.cefr`, Dexie v4 + `upgrade()`, `UserSettings.levelProfile`/`sessionSize`, `getProfile()` safe-parse, `SkippedRepository` (bảng `skipped` có từ v1, chưa từng có repository) | ✅ Xong | `lib/domain/{word,user}.ts`, `lib/db/{dexie,rows}.ts`, `lib/repositories/dexie/{user,skipped}-repository.ts` |
| 9.2 | Kho từ vựng: `scripts/{corpus-source-data,build-corpus}.ts` → `public/corpus/v1/**`, `lib/corpus/{types,load,pick,exclude}.ts`, cache header `next.config.ts` | ✅ Xong — xem ADR-015 (nguồn tự biên soạn, không phải NGSL/NAWL/BSL như dự tính ban đầu) | `lib/corpus/**`, `public/corpus/v1/**` |
| 9.3 | `lib/level/{cefr,placement,srs-signal,resolve}.ts`, mở `level` lên A1..C2, `stores/level-store.ts`, nối `analyzeWork.summary.estimatedLevel` + tín hiệu SRS | ✅ Xong | `lib/level/**`, `stores/level-store.ts` |
| 9.4 | `components/TriageList.tsx`, `app/(stack)/placement/page.tsx`, nhánh first-run ở Home | ✅ Xong — đã sửa thêm 1 bug phát hiện lúc test bằng trình duyệt thật: `seedIfEmpty()` (5 từ demo cũ) đua với và luôn thắng CTA placement mới, đã bỏ lời gọi tự động (xem ADR-018) | `app/(stack)/placement/page.tsx`, `components/TriageList.tsx`, `app/providers.tsx` |
| 9.5 | `enrichWordBatch` (task/route), `stores/topup-store.ts`, mệnh đề `isEligible('recall')` mới, `caps` từ `navigator.onLine`, `sessionSize` chạy thật | ✅ Xong — đã xác minh bằng trình duyệt thật (không có `GEMINI_API_KEY` cấu hình trong dev env, nên đây cũng là bằng chứng sống cho đường degraded): notebook rỗng → 5 từ được ghi (4 B2 + 1 C1 theo đúng tỉ lệ 70/30), `recall` render đúng | `lib/ai/tasks/{contracts,registry.server}.ts`, `app/api/ai/enrich-batch/route.ts`, `stores/topup-store.ts`, `lib/srs/session.ts` |
| 9.6 | Quyết định số phận `analyzeDocument`/`gradeSentence.rewriteProfessionally` mồ côi, viết bù ADR-014, thêm ADR-015..020, cập nhật docs | ✅ Xong — xoá `rewriteProfessionally` (cô lập, an toàn); **giữ nguyên** `analyzeDocument` + `ImportRepository.setCandidates/setTriage` (rủi ro đụng schema `Import` dùng chung nhiều `kind`, phạm vi một tính năng riêng) — xem ADR-020 | `docs/decision.md`, `lib/ai/tasks/{contracts,registry.server}.ts` |

**Nợ để lại có chủ đích (không phải bỏ sót):** `analyzeDocument` vẫn mồ côi (ADR-020); corpus v1 là tự biên soạn ~190 từ, không phải NGSL/NAWL/BSL thật (ADR-015, có đường dẫn rõ ràng để thay bằng dữ liệu thật sau); van chống-phình top-up (60s cooldown, 20 từ/ngày) là số tuỳ chọn chưa có dữ liệu thật hiệu chỉnh (ADR-018).
