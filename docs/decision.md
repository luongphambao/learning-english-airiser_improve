...
**Hệ quả:** `01-PROJECT-SPEC (2).md` dòng Non-goals được sửa để trỏ về ADR này thay vì cấm tuyệt đối. Không có migration dữ liệu — mascot không đọc/ghi trường `UserSettings` mới nào. Đánh đổi đã biết: chưa có cờ tắt mascot trong Cài đặt (nếu người dùng thấy phiền, bước tiếp theo là thêm `settings.showMascot: boolean`); rồng vẽ tay bằng shape cơ bản, không phải minh hoạ artist-made, nên độ chi tiết "anime" ở mức chibi/flat chứ không phải vẽ tay chi tiết.
